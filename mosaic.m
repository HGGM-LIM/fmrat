...
nSPM=2;    
 ...
 for i=1:nSPM
    ...
    xSPM.Ic         =   i;
    %SPM.xCon.Ic=i;
    %        [SPM, SPMVol,p,k]=getSPM(path_SPM,cname{i},cons{i});
    
%     SPMVol(i).XYZ={};
    [SPM, SPMVol]	=   spm_getSPM(xSPM);
    
%    eval(sprintf('save SPM%d.mat SPM;',i));
    switch ver
            case 'spm99'
                Z               =   SPM.Z;
...
%                 jSPMo(j_igual) = 0.01;                
    ...
    if (mRow==0)&&(mCol==0)
        if nSPM>0
        text(0.0,1.4 , ['T MAP'], 'Units','normalized',...
            'FontSize', 14*sc,'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');  
            text(0.0,1.2 , ['Functional: ' SPMVol(1,1).swd filesep 'SPM.mat'], 'Units','normalized',...
            'FontSize', 7*sc,'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');        
        end
        text(0.0,1.15 , ['Background: ' Vbg.fname ], 'Units','normalized',...
            'FontSize', 7*sc,'HorizontalAlignment', 'left', 'VerticalAlignment', 'top'); 
    end
    %***********************************************************************************************************************************
    axis off;
    ...
    if (mRow==0)&&(mCol==0)
        if nSPM>0
        text(0.0,1.4 , ['Z MAP'], 'Units','normalized',...
            'FontSize', 14*sc,'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');              
        text(0.0,1.2 , ['Functional: ' SPMVol(1,1).swd filesep 'SPM.mat'], 'Units','normalized',...
            'FontSize', 7*sc,'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');        
        end
...
% if nSPM == 2 
%    hAx = axes('Position', [.02 fVert .08 12/WIN(4)]);
%    image(193);
%    axis off;
% end;
...
set(Fgraph,'Position',[320 40 1400 990]);
set(Fgraph,'PaperPositionMode','auto');
if nargin~=0
    print(Fgraph,'-dtiff','-r0',[path filesep 'results_' nam '_' num2str(fwe) '_p_' ...
        regexprep(num2str(p),'\.','_') '_k_' num2str(kl) '.tif'])
else
    print(Fgraph,'-dtiff','-r0',[path filesep 'results_' nam '_manual_' num2str(fwe) '_p_' ...
        regexprep(num2str(p),'\.','_') '_k_' num2str(kl) '.tif'])
end
%--------------------------------------------------------------------------
figure(hZmap);
...
set(hZmap,'Position',[320 40 1400 990]);
set(hZmap,'PaperPositionMode','auto');
if nargin~=0
    print(hZmap,'-dtiff','-r0',[path filesep 'Zresults_' nam '_' num2str(fwe) '_p_' ...
        regexprep(num2str(p),'\.','_') '_k_' num2str(kl) '.tif'])
else
    print(hZmap,'-dtiff','-r0',[path filesep 'Zresults_' nam '_manual_' num2str(fwe) '_p_' ...
        regexprep(num2str(p),'\.','_') '_k_' num2str(kl) '.tif'])
end
