...
%      function named CALLBACK in FMRI_NIFTI_GUI.M with the given input arguments.
%
...
% Last Modified by GUIDE v2.5 11-Aug-2014 15:19:05
...
if eval(ok)        
    ...
    elseif block_ok==0
        set(handles.uipanel13,'BackgroundColor',[0.847,0.161,0]);
        set(hObject,'Value',0);  
...
ok= ['~isempty(data) && isdir(sel_dir) && (block_ok==1) && ' ...
    '((coreg && (sp~=0))|| (coreg && (~isempty(atlas_dir))) || (~coreg))' ...
    '&& ((custom_resol==0)||((~isempty(rx))&&(~isempty(ry))&&(~isempty(rz))))' ...
...
if eval(ok)        
    ...
    if file ~=0 
        ...
        try
            fullpath=[path file];
            save(fullpath,'sel_dir','data','block_ok','NR','Nrest','Nstim',...
   ...
   set(handles.uipanel17 ,'Visible','off');  
    ...
    if ~isempty(NR) && ~isempty(Nrest) && ~isempty(Nstim) && ...
       NR>(Nrest+Nstim) && (mod(NR,(Nrest+Nstim)) == Nrest)
            ...
            set(handles.uipanel11,'BackgroundColor',[0.906,0.906,0.906]);
            set(handles.text26,'BackgroundColor',[0.9,0.7,0.7]);
    else
        axes(handles.axes3); 
        cla;
...
% handles    structure with handles and user data (see GUIDATA)
    install     =   mfilename('fullpath');
    roi_dir     =   get(handles.edit27,'String');
